...
class Robot:
    ...
    # --- AGREGADO TP07 EXTENSION (autorizado por el docente): antes
    # 'festejar' solo mandaba el Siu de Cristiano Ronaldo. Ahora recibe
    # el NOMBRE del gesto ("siu", "messi", "decepcion", ...) para no
    ...
    def festejar(self, nombre: str = "siu", duracion: float = 3.0) -> EstadoRobot:
        ...
        El robot fisico no tiene esta accion: su bridge no expone
        'Festejo'. Si esto llegara al laboratorio, getattr devuelve None
        y falla aca, antes de mandar nada. Fail-closed a proposito.
        """
        self._exigir_conexion()
        metodo = getattr(self._cliente, "Festejo", None)
        if metodo is None:
