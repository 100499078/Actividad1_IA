...
class Mundo:
    ...
    def gesto(self, nombre: str, duracion: float = 2.0) -> None:
        with self._lock:
            ...
            self._gesto_hasta = ahora + duracion
            # --- AGREGADO TP07 EXTENSION (autorizado por el docente) ---
            # Desde cuando esta activo este gesto. Lo usa el renderer
            # (arrancar.py) para el resorte de piernas del aterrizaje:
            # un pulso corto que arranca en 0 y solo tiene sentido medido
            # desde el instante en que el gesto empezo, no desde el reloj.
            self._gesto_desde = ahora
    ...
    def leer(self) -> dict:
        with self._lock:
            return {
                ...
                "fase": self.fase,
                # AGREGADO TP07 EXTENSION: segundos desde que arranco el
                # gesto actual. Solo importa mientras dura el resorte de
                # aterrizaje (ver _impulso() en arrancar.py); despues, un
                # numero grande no le hace nada a nadie.
                "gesto_t": round(time.monotonic() - self._gesto_desde, 4),
            }
